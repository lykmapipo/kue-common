...
const redisUrl = () => {
  ...
  url = env.getString('KUE_REDIS_URL') || env.getString('REDIS_URL') || url;
  return url;
};
...
const withDefaults = optns => {
  // merge defaults
  let options = common.mergeObjects(
    {
      timeout: env.getNumber('KUE_TIMEOUT', 5000),
      concurrency: env.getNumber('KUE_CONCURRENCY', 10),
      attempts: env.getNumber('KUE_MAX_ATTEMPTS', 3),
      priority: env.getString('KUE_PRIORITY', 'normal'),
      backoff: { type: 'exponential' },
      jobEvents: env.getBoolean('KUE_JOB_EVENTS', false),
      jobsPath: env.getString('KUE_JOBS_PATH', `${process.cwd()}/jobs`),
      removeOnComplete: env.getBoolean('KUE_REMOVE_ON_COMPLETE', true),
      redis: redisUrl(),
      httpPort: env.getNumber('KUE_HTTP_PORT', 5000),
      httpUsername: env.getString('KUE_HTTP_USERNAME', 'kue'),
      httpPassword: env.getString('KUE_HTTP_PASSWORD', 'kue'),
      httpEnabled: env.getBoolean('KUE_HTTP_ENABLED', true),
    },
    optns
  );

  // compact and return
  options = common.compact(options);
  return options;
};
...
const dispatch = (optns, cb) => {
  ...
  const definition = common.mergeObjects(jobs[type], options); // TODO no job def?
...
const start = optns => {
  ...
  const types = common.compact(optns && optns.types ? [].concat(optns.types) : []);
...
const listen = (optns, cb) => {
  ...
  app.use(httpServer);
  if (httpEnabled) {
    app.listen(httpPort, error => done(error, common.mergeObjects(options)));
  } else {
    done(null, common.mergeObjects(options));
  }
...
exports.FAILED_ATTEMPT = FAILED_ATTEMPT;
exports.JOB_COMPLETE = JOB_COMPLETE;
exports.JOB_ENQUEUE = JOB_ENQUEUE;
exports.JOB_FAILED = JOB_FAILED;
exports.JOB_FAILED_ATTEMPT = JOB_FAILED_ATTEMPT;
exports.JOB_PROGRESS = JOB_PROGRESS;
exports.JOB_PROMOTION = JOB_PROMOTION;
exports.JOB_QUEUED = JOB_QUEUED;
exports.JOB_REMOVE = JOB_REMOVE;
exports.JOB_START = JOB_START;
exports.PRIORITY_CRITICAL = PRIORITY_CRITICAL;
exports.PRIORITY_HIGH = PRIORITY_HIGH;
exports.PRIORITY_LOW = PRIORITY_LOW;
exports.PRIORITY_MEDIUM = PRIORITY_MEDIUM;
exports.PRIORITY_NORMAL = PRIORITY_NORMAL;
...
exports.onJobEnqueue = onJobEnqueue;
exports.onJobFailed = onJobFailed;
exports.onJobFailedAttempt = onJobFailedAttempt;
exports.onJobProgress = onJobProgress;
exports.onJobPromotion = onJobPromotion;
exports.onJobQueued = onJobQueued;
exports.onJobRemove = onJobRemove;
